...
class SimpleNetworkAD(nn.Module):
    ...
    def loss(self):
        ...
        f_n = N_t - (self.general_para[6]*torch.sigmoid(T)+ self.general_para[7]*torch.sigmoid(A) +self.general_para[8]*torch.sigmoid(A*T))
...
def test_ad(model, args, config, now_string, param_ls, param_true, show_flag=True, gt=None, loss_2_details=None):
    ...
    N_AonN = param_ls[7]* sigmoid(A)
    N_TonN = param_ls[6] * sigmoid(T)
    N_ATonN = param_ls[8] * sigmoid(A*T)
    ...
    N_AonN_matlab = param_true[7]* sigmoid(A)
    N_TonN_matlab = param_true[6] * sigmoid(T)
    N_ATonN_matlab = param_true[8] * sigmoid(A*T)
    ...
    m.add_subplot(x_list=x, y_lists=[A_TonA, A_prod, A_TonA_matlab, A_prod_matlab],
                  color_list=['b', 'r', 'b', 'r'], fig_title="Euqation A",
                  line_style_list=["solid", "solid", "dashed", "dashed"],
                  legend_list=["A_TonA", "A_prod", "A_TonA_matlab", "A_prod_matlab"], line_width=6, fig_title_size=40,x_label_size=20,
    ...
    #               legend_list=[], line_width=6, fig_title_size=40,x_label_size=20,
    # y_label_size=20, fig_x_label = "time",
    # fig_y_label = "influence")
    #
