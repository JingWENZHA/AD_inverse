...
class GroundTruthAD:
    def __init__(self, t_max, length):
        ...
        print("self.y_true: {} \n".format(self.y_true))
        print("FGHJKHGUHFYGUH", timestep/8,timestep)

        self.y_true_month = torch.tensor([[0, math.floor(timestep/8), math.floor(timestep/6), math.floor(timestep/3), math.floor(timestep/2)],
                                          [math.floor(timestep/7), math.floor( timestep/6), math.floor(timestep/3), math.floor(timestep/2), math.floor(timestep/1.3)],
                                          [math.floor(timestep/7), math.floor( timestep/6), math.floor( timestep/2.4), math.floor(timestep/1.3), math.floor(timestep-10)]])
...
class SimpleNetworkAD(nn.Module):
    ...
    def loss(self):
        ...
        y_totrain = torch.ones([5,3]).float()
        y_totrain[:,0] = torch.index_select(y[:,0], 0, self.gt_ytrue_month[0])
        y_totrain[:,1] = torch.index_select(y[:,1], 0, self.gt_ytrue_month[1])
        y_totrain[:,2] = torch.index_select(y[:,2], 0, self.gt_ytrue_month[2])
...
def test_ad(model, args, config, now_string, param_ls, param_true, show_flag=True, gt=None, loss_2_details=None):
    ...
    for i in range(3):
        ...
        ax.scatter(x = (model.gt_ytrue_month[i,:].cpu().detach().numpy())/10, y = model.gt_ytrue[:,i].cpu().detach().numpy(), color = colorlist[i], marker = 'x', s = 400, linewidths= 6)
