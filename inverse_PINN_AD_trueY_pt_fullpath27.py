...
class SimpleNetworkAD(nn.Module):
    def __init__(self, config,num_sub):
        ...
        self.personalized_para = nn.Parameter(torch.abs(torch.rand(28)))
    ...
    def loss(self):
        ...
        f_a = A_t - (self.personalized_para[0] - self.personalized_para[1]*A  #   A metabolism
                     + (self.personalized_para[2]*torch.pow(T,2)) / (torch.pow((self.personalized_para[3]),2) + torch.pow(T,2)) # T on A
                     + (self.personalized_para[5]*torch.pow(N,2)) / (torch.pow((self.personalized_para[6]),2) + torch.pow(N,2)))  # N on A
                     #- self.personalized_para[8]*torch.matmul(A,self.Laplacian))

        f_t = T_t - (self.personalized_para[9] - self.personalized_para[10]*T
                + (self.personalized_para[11]*torch.pow(T,2)) / (torch.pow((self.personalized_para[12]),2) + torch.pow(T,2)) # T on A
                + (self.personalized_para[14]*torch.pow(N,2)) / (torch.pow((self.personalized_para[15]),2) + torch.pow(N,2)))  # N on A
                #- self.personalized_para[17]*torch.matmul(A,self.Laplacian))

        f_n = N_t - (((self.personalized_para[18]*torch.pow(A,2)) / (torch.pow((self.personalized_para[19]),2) + torch.pow(A,2)) # T on A
                + (self.personalized_para[21]*torch.pow(T,2)) / (torch.pow((self.personalized_para[22]),2) + torch.pow(T,2)) # T on A
                + (self.personalized_para[24]*(torch.pow(A,2)+ torch.pow(T,2))) / (torch.pow((self.personalized_para[25]),2) + torch.pow(A,2) + torch.pow(T,2))))  # N on A
        ...
        loss_4 = self.loss_norm(torch.abs(self.personalized_para), self.personalized_para) * 1e10
        # print(loss_4)
...
def train_ad(model, args, config, now_string):
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        if epoch % args.save_step == 0:
            test_ad(np.asarray(loss_record), model, args, config, now_string, True, model.gt, None)
    ...
    myprint("NUM_SUB: {}------------------------- FINISHED TRAINING ---------------------------------------------".format(model.num_sub), args.log_path)

    [equA, equT, equN] = test_ad(np.asarray(loss_record), model, args, config, now_string, True, model.gt, None)

    myprint("[Loss]", args.log_path)
    myprint("True parameter : {};".format(model.true_para), args.log_path)
    myprint("General parameter estimation: {};".format(model.personalized_para), args.log_path)
    ...
    return model, res_dic, para_pred, np.concatenate((np.mean(equA, 1),np.mean(equT, 1),np.mean(equN, 1)))
...
def test_ad(loss_list,model, args, config, now_string, show_flag=True, gt=None, loss_2_details=None):
    # print("--------------------------------------------------call test ad--------------------------------------------------")
    model.eval()
    ...
    for i in range(3):
        ax =m.add_subplot(
                y_lists=[y[:, i].reshape(num_timestep) for j in range(184)],  # y_lists=[y[:,1:3]]
                x_list=x,
    ...
    equN_AT = (param_ls[24] * (np.power(A, 2) + np.power(T, 2))) / (
                np.power((param_ls[25]), 2) + np.power(T, 2) + np.power(A, 2))
    ...
    equA = np.array(
        [abs(equA_prod.reshape(num_timestep)), abs(equA_degr.reshape(num_timestep)), abs(equA_T.reshape(num_timestep)),
         abs(equA_N.reshape(num_timestep))])
    equT = np.array(
        [abs(equT_prod.reshape(num_timestep)), abs(equT_degr.reshape(num_timestep)), abs(equT_A.reshape(num_timestep)),
         abs(equT_N.reshape(num_timestep))])
    equN = np.array(
        [abs(equN_A.reshape(num_timestep)), abs(equN_T.reshape(num_timestep)), abs(equN_AT.reshape(num_timestep))])

    equA = equA / np.sum(equA, axis=0)
    equT = equT / np.sum(equT, axis=0)
    equN = equN / np.sum(equN, axis=0)

    ax1 = m.add_subplot(x_list=x, y_lists=[equT_prod], color_list=['w'], fig_title="Equation A",
                        line_style_list=["solid"], line_width=0.1, fig_title_size=40, x_label_size=40, y_label_size=40,
                        fig_x_label="time", fig_y_label="% influence")

    ax1.stackplot(x, equA, labels=["equA_prod", "equA_degr", "equA_T", "equA_N"],
                  colors=["palegoldenrod", "lightblue", "cadetblue",
                          "steelblue"])  # cornflowerblue palegoldenrod mediumaquamarine
    ax1.legend(loc='lower left', fontsize=20)

    ax2 = m.add_subplot(x_list=x, y_lists=[equT_prod], color_list=['w'], fig_title="Equation T",
                        line_style_list=["solid"], line_width=0.1, fig_title_size=40, x_label_size=40, y_label_size=40,
                        fig_x_label="time", fig_y_label="% influence")
    ...
    ax3 = m.add_subplot(x_list=x, y_lists=[equT_prod], color_list=['w'], fig_title="Equation N",
                        line_style_list=["solid"], line_width=0.1, fig_title_size=40, x_label_size=40, y_label_size=40,
                        fig_x_label="time", fig_y_label="% influence")
...
def run_ad_truth(opt):
    ...
    for i in range(100):
        config = ConfigAD()
        model = SimpleNetworkAD(config,i+83).to(device)
        model, tmp, para, tmp_infl = train_ad(model, args, config, now_string)
        personalized_para.append(para.cpu().detach().numpy());
        models.append(model)
        influence.append(tmp_infl)
